...
is_sm70 = torch.cuda.get_device_capability("cuda") == (7, 0)
...
@pytest.mark.parametrize("dtype", ([torch.float16] if (is_sm75 or is_sm70) else [torch.float16, torch.bfloat16]))
...
@pytest.mark.parametrize("dtype", ([torch.float16] if (is_sm70 or is_sm75) else [torch.float16, torch.bfloat16]))
# @pytest.mark.parametrize('dtype', [torch.float16])
@pytest.mark.parametrize("deterministic", [False, True])
